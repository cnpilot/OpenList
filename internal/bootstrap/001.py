...
2026-05-19 - 功能新增适配自动发种脚本
  ...
  - 改用 rclone move 稳妥将完整原种子目录整体移动至 /home/boxbox/finish
...
2026-05-19 - 优化修复二：强制执行顺序+无条件移动规则定稿
  - 固定执行顺序：torcp执行完毕 → rclone执行完毕 → 最后再移动原目录至finish
  - 新增规则：无论torcp是否刮削成功、无论rclone是否完成转移，都不阻拦原目录移入finish
  - 所有子流程全部设置check=False，流程不中断、不终止、不卡任务
2026-05-19 - 优化修复三：ISO文件独立拦截规则
  - 目录存在ISO文件直接全程拦截
  - 跳过torcp刮削、跳过媒体转移、同时跳过原目录归档移入finish
  - 仅打印日志提示，不执行任何操作
2026-05-20 - 最终修复
  - 修复移动路径：自动移动外层 HHclub/HDSky/全站点外层总目录
  - 修复日志路径：全部日志统一放在 logs_bdmv，不污染 finish
