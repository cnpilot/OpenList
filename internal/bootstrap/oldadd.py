...
class QBittorrentClient:
    ...
    def generate_path_prefix(self, title, site_name):
        ...
        site_tag = site_name.lower()

        if f'@{site_tag}' in title_lower:
            return f"{site_name}_Bluray"
        elif 'remux' in title_lower:
            return f"{site_name}_Remux"
        elif '10bit' in title_lower:
            return f"{site_name}_10bit"
        elif 'web-dl' in title_lower:
            return f"{site_name}_Web"
        else:
            return f"{site_name}_Unknown"
            ...
            try:
                ...
                if site_config['name'].lower() == "hhclub":
                    if douban_id != "No_Douban":
                        ...
                        qb_save_path = os.path.join(self.download_base_path, f"{path_prefix}_{id_string}_{db_id}")
                    else:
                        qb_save_path = os.path.join(self.download_base_path, f"{path_prefix}_{id_string}_NoDouban")
                else:
                    qb_save_path = os.path.join(self.download_base_path, f"{path_prefix}_{id_string}_{imdb_id}")
